                       ...
                       uint16_t lengthPacket )
{
    ...
    ASSERT_ID( eGrPS_EcanDrv, ePS_ErrorParameters,
    ...
    pEcan->pEcanRegs->CTRL1bits.CANCKS = 1;  // Выбор частоты Fcan = 2 * 59,904 MHz = 119,808 MHz
    ...
    pEcan->pEcanRegs->CFG1bits.BRP     = 7;  // Определяем Tq (квант времени) равным 119.808 MHz / 2 / 8 = 135/2 = 7,488 MHz 
                                            // Номинальное время передачи бита (Nominal Bit Time - NBT) 
                                            // = 8=**q = (936000 бит/с)
    pEcan->pEcanRegs->CFG2bits.PRSEG    = 1;
    pEcan->pEcanRegs->CFG2bits.SEG1PH   = 1;
    pEcan->pEcanRegs->CFG2bits.SEG2PH   = 2;
    pEcan->pEcanRegs->CFG2bits.SAM      = 1;
    pEcan->pEcanRegs->CFG2bits.SEG2PHTS = 0;
    ...
    pEcan->pEcanRegs->RXM0SID = (0x7FF << 5);     
    ...
    switch ( DRV_DATA( self )->transmissionState ) {
        ...
        case eEcanTrStateInProgress:
           if( DRV_REGS( self )->TR01CONbits.TXREQ0 == 0 ) { // ждем окончания передачи
                ...
                {
                    DRV_DATA( self )->error = ePS_NoError;
                }
                else
                {
                    DRV_DATA( self )->error = ePS_ErrorDevice;
                }
                DRV_DATA( self )->transmissionState = eEcanTrStateIdle;               
            }
            break;
