...
/// \param reg - имя регистра периферийного устройства (например, \a C1TXD);
/// \param arrayRAM - имя буфера ОЗУ или ПЗУ;
/// \param localAddress - имя локальной переменной для конфигурации адреса, тип \a uint32_t.
///
#define ECAN_CONFIG_DMA( dma, direct, irqSel, reg, arrayRAM, localAddress )                                          \
    do                                                                                                               \
    {                                                                                                                \
    dma##CONbits.SIZE  = 0x0;                             /* Поток в словах (16 бит) */                              \
    dma##CONbits.DIR   = direct;                          /* Направление потока */                                   \
    dma##CONbits.AMODE = 0x2;                             /* Косвенная адресация (адрес) переферийнога устройства */ \
    dma##CONbits.MODE  = 0x0;                             /* Один блок транслируется в/из буфера */                  \
