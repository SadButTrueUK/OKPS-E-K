...
void checkCauseReset_run( void )
{
    ...
    //  входу MCLR. Все другие признаки СБРОСа считаются некорректными.
    ASSERT_EX_ID( eGrPS_Main, ePS_MainIncorrectReset, 
                  (_POR || _EXTR) && !_WDTO && !_TRAPR && !_SWR && !_IOPUWR, 
                  RCON, 0, 0, 0 );

    RCON &= ~_RCON_POR_MASK & ~_RCON_EXTR_MASK & ~_RCON_BOR_MASK;               // Биты в регистре RCON должны сбрасываться программно.
}
