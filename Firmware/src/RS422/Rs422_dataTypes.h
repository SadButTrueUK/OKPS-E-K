...
//*****************************************************************************
/// \brief Длина массива для чтения данных из регистра UART.
///
#define V_ARRAY_IN_REG  64 //tret 13.01.2020
...
//*****************************************************************************
/// \brief Коды критических ошибок по тайм-ауту.
///
typedef enum 
{
...
typedef struct 
{
    uint8_t    cInReadRegTime;                                ///< Счетчик для фиксации времени  чтения регистра UART/
    uint8_t    aInReadRegTime[V_ARRAY_IN_REG];                ///< Массив времен данных при чтении регистра UART.
    uint8_t    aInReadReg[V_ARRAY_IN_REG];                    ///< Массив считанных данных с регистра UART.
    uint8_t    ndxReadReg;                                    ///< Текущий индекс чтения массива aInReadReg.
    uint8_t    ndxWriteReg;                                   ///< Текущий индекс записи массива aInWriteReg.
...
typedef enum 
{
    ...
    errorRS422_noSynchroTele1,                       ///< нет синхронизации после получения 1-й телеграммы               
    ...
    errorRS422_noSynchroTele2,                       ///< нет синхронизации после получения 2-й телеграммы
    errorRS422_timeOfTransmitionIsOver,              ///< время отправки телеграммы вышло, телеграмма не отправлена
    errorRS422_noSynchroAfterTransmition             ///< нет синхронизации после отправки телеграмм} eRs422_error;
} eRs422_error;
...
typedef struct CommunicationRS422_Tag 
{
    ...
    uint16_t         noExchangeCnt;                     ///< Счетчик тайм-аута отсутствия обмена.
    uint16_t         noExchangeCtrlSystemCnt;           ///< Счетчик тайм-аута отсутствия обмена c УС.
    uint8_t timeOut;                                    ///< Переменная тайм-аута.
} CommunicationRS422;
...
typedef struct 
{
    ...
    uint16_t noExchangeTime;                  ///< Тайм-аут отсутствия обмена.
    uint16_t noExchangeTimeCtrlSystem;        ///< Тайм-аут отсутствия обмена c УС.
} StateRS422;
