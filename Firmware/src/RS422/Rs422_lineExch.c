...
{
    lineStr -> iniUart( );
    inRS422_ctor( &lineStr -> inRS422 );
    outRS422_ctor( &lineStr -> outRS422 );
...
void Rs422_lineRun( CommunicationRS422 *lineStr, StateRS422 *totalStr )
{
    ...
    //Декремент таймаута отсутствия обмена
    if( lineStr -> noExchangeCnt )
        lineStr -> noExchangeCnt--;
    else
        lineStr -> newOrderEvent = false;
    //Декремент таймаута отсутствия обмена с УС
    if( lineStr -> noExchangeCtrlSystemCnt ) lineStr -> noExchangeCtrlSystemCnt--;

    //Декремент переменной таймаута
    if( lineStr -> timeOut != 0 ) lineStr -> timeOut--;
    ...
    switch( lineStr -> state )
    {
        ...
        case eRs422exec_1stTeleReception: //1            // Ожидание приема 1-ой телеграммы
            ...
            //Установка таймаута передачи ответной телеграммы
            ...
            //Установка таймаута контроля синхронизации
        ...
        case eRs422exec_1stTeleSynchro: //2      //Проверка синхронизации 1-ой телеграммы
            ...
            { //Была принята короткая телеграмма
                ...
                //Установка таймаута контроля передачи       
            ...
            { //Была принята длинная телеграмма
                ...
                //Установка таймаута на прием синхронизированных данных
                lineStr -> timeOut = TIME_OUT_SYNC_TELE;
...
void Rs422_lineInterrupt( CommunicationRS422 *lineStr, StateRS422 *totalStr )
{
    inRS422_run( &lineStr -> inRS422, totalStr -> address );
    outRS422_run( &lineStr -> outRS422 );
}
