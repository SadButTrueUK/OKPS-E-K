...
typedef enum
{
    ...
    eGrPS_EcanDrv              = 0x03,    ///< отказы драйвера модуля \a CAN
    ...
    eGrPS_ManageCanDrv         = 0x16     ///< отказы менеджера драйвера модуля \a CAN  
} ProtectionStateGroup;


typedef enum
{
    ePS_NoError         = 0,                    ///< нет ошибок
    ePS_ErrorParameters = 1,                    ///< ошибка параметров вызова функций        
    ePS_BufOverflow     = 2,                    ///< переполнение буфера 
    ePS_ErrorDevice     = 3,                    ///< неисправность устройства         
    ePS_TimeOut         = 4,                    ///< тайм-аут выполнения         
    ePS_NoAck           = 5,                    ///< нет подтверждения операции  
    ePS_ErrorAlg        = 6,                    ///< ошибка алгоритмов
    ePS_NoInic          = 7                     ///< не было инициализации             
} TypicalErrors;
...
typedef enum
{
    ePS_AdcOverflow   = 1,              ///< очередь драйвера переполнилась
    ePS_AdcErrorDMA   = 2               ///< ошибка в работе DMA. Не все транзакции выполнены 
...
typedef enum 
{
    ...
    ePS_FaultU3_3V       = 1,                  ///< ошибка при проверке питания 3.3 В
...
* 
* Изменения:
* Изменены коды отказов ProtectionStateAdcDrvId, т.к. АЦП работает с DMA
*/
