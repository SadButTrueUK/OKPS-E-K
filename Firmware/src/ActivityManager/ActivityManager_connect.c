...
typedef struct ExchangeDataStr
{
    ...
    uint8_t s4_s5  :2;         ///< Состояние контроля положения стрелки.
...
void ActiveManagerConnect_run( ActivityManager_DeviceStr *activityManager, BlockExch_Str *blockExch )
{
    ...
    //Установить состояние положения стрелки соседа 
    ActivityManager_setNeighborPositionShunt( activityManager, neighborState.str.s4_s5 );
...
static void ownStateForming( ActivityManager_DeviceStr *activityManager, BlockExch_Str *blockExch, uExchangeDataStr *st )
{
    ...
    //Получить положение стрелки своего прибора
    st->str.s4_s5 = ActivityManager_getOwnStatePosShift( activityManager );
