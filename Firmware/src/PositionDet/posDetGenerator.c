...
//*****************************************************************************
/// \brief Значения максимально допуустимого напряжения обратной связи генератора (+10% от номинального)
///
...
static uint16_t          t_o_cnt;                         ///< Счетчик таймаута
...
/// \details Данные считаются актуальными только тогда, когда истёк таймаут DEVICE_HAVE_NOT_ACTUAL_DATA в одном из следующих случаев:
/// 1. После включения генератора;
/// 2. При изменении уровня напряжения в приказе от управляющей системы.
/// Также данные могут обрабатываться, когда окончен таймаут по включению питания STARTUP_TIMEOUT
///
...
static void checkGeneratorData( void )
{
    ...
    static const uint16_t DEVICE_HAVE_NOT_ACTUAL_DATA = 320;
    enum intStates_t
    {
        eOff,
        eOn,
...
void PosDetGenerator_setLevel ( uint8_t val ) 
{
    if( val ) //если передан 0, то устанавливаемое напряжение не изменяется
    {
        ...
        //при изменении величины уровня в приказе перезаписать значение и уйти на установку таймаута ожидания готовности данных 
...
void PosDetGenerator_checkUfb( void )   //!!!доделать
{
