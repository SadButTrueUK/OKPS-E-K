...
/// \param data - уазатель на результаты измерения.
/// \return Положение.
///
...
void FiveWireECPosDet_run( void )
{
    if( flags.str.ctrl )
    { // Включено
        if( measurToCnt ) measurToCnt--; // Декремент таймаута измерения
        if( changePosDetCnt ) changePosDetCnt--; // Декремент таймаута смены положения
        // Проверка таймаута измерений
        if( measurToCnt == 0 )
        { // Запись измеренных значений
            ...
            { // Положение определено 
                ...
                changePosDetCnt = CHANGE_POS_DET_TO; // Обновить таймаут изменения
            }
            ...
            { // Положение не определено и пришло время сменить положение измерения
                ...
                changePosDetCnt = CHANGE_POS_DET_TO; // Взвести таймаут следующего изменения
            }
            measurToCnt = MEASUR_TO; // Взвести таймаут следующего измерения
        }
    }
}
